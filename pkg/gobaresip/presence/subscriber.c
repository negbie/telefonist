...
static struct list presencel = LIST_INIT;
			   ...
			   void *arg)
{
	...
	if (!type_hdr) {
		...
		if (length_hdr && 0 == pl_strcmp(&length_hdr->val, "0")) {
	...
	if (!type_hdr ||
	    0 != pl_strcasecmp(&type_hdr->val, "application/dialog-info+xml")) {
	...
	if (msg && msg->mb && !re_regex((const char *)mbuf_buf(msg->mb),
				   mbuf_get_left(msg->mb),
				   "<state[ \t]*>[^<]+</state[ \t]*>",
				   NULL, &pl, NULL)) {

		if (0 == pl_strcasecmp(&pl, "confirmed") ||
		    0 == pl_strcasecmp(&pl, "early") ||
		    0 == pl_strcasecmp(&pl, "proceeding") ||
		    0 == pl_strcasecmp(&pl, "trying")) {
			status = PRESENCE_BUSY;
		}
		else if (0 == pl_strcasecmp(&pl, "terminated")) {
...
static int subscribe(struct presence *pres)
{
	...
	debug("presence: subscribe starting for contact %s\n",
	      contact_uri(pres->contact));
	...
	debug("presence: subscribe %s: err=%d sub=%p\n",
	      contact_uri(pres->contact), err, pres->sub);
	if (err) {
...
static int presence_alloc(struct contact *contact)
{
	...
	debug("presence: presence_alloc success for contact %s (pres=%p)\n",
	      contact_uri(contact), pres);
